...
def classify(caffemodel, deploy_file, image_files,
    ...
    for i, image_results in enumerate(scores):
        print '==> Image #%d' % i
	imgD = PIL.Image.fromarray(images[i])
	strBBox = ""
	nObj = 0
	    ...
	    dr = PIL.ImageDraw.Draw(imgD)
	    dr.line(((pL,pT),(pR,pT)),col,thickness)
	    dr.line(((pR,pT),(pR,pB)),col,thickness)
	    dr.line(((pR,pB),(pL,pB)),col,thickness)
	    dr.line(((pL,pB),(pL,pT)),col,thickness)
	    #dr.rectangle(((round(left),round(top)),(round(right),round(bottom))), outline=(255,0,0))
	    strBBox += "tomato " + str(pL) + " " + str(pT) + " " + str(pR) + " " + str(pB) + "\n"
	
	font = PIL.ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 100)
	dr.text((12, 12), str(nObj), font=font, fill='#ffffff')
	dr.text((10, 10), str(nObj), font=font, fill='#ff0000')
#	imgD.save("/home/ubuntu/Downloads/tomato_demo/"+str(i)+".png")

	fileBBox = open("/home/ubuntu/Downloads/tomato_val/tomato_val_"+str(i+1)+".txt", "w");
	fileBBox.write(strBBox);
	fileBBox.close();
