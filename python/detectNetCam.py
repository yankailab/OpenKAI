...
def classify(caffemodel, deploy_file, image_files,
    ...
    while(True):
        ...
        print '=====================Script1 took %f seconds.' % (time.time() - script_start_time,)
        scores = forward_pass(image, net, transformer, batch_size=batch_size)
        print '=====================Script2 took %f seconds.' % (time.time() - script_start_time,)
        print "scores zeros:", np.count_nonzero(scores)#, scores
        ### Process the results
        if np.count_nonzero(scores)!=0:
            nObj = 0
            for left, top, right, bottom, confidence in scores[0]:
        ...
        print '=====================Script3 took %f seconds.' % (time.time() - script_start_time,)
...
if __name__ == '__main__':
    ...
    classify(args['caffemodel'], args['deploy_file'],
