...
#include "_DNNCaffeFCN.h"
namespace kai
{
_DNNCaffeFCN::_DNNCaffeFCN()
{
	num_channels_ = 0;
	batch_size_ = 0;
}
...
void _DNNCaffeFCN::setup(const string& model_file, const string& trained_file,
...
void _DNNCaffeFCN::SetMean(const string& mean_file)
{
...
std::vector<vector<Prediction> > _DNNCaffeFCN::ClassifyBatch(
...
std::vector<float> _DNNCaffeFCN::PredictBatch(const vector<cv::Mat> imgs)
{
	Blob<float>* input_layer = net_->input_blobs()[0];
...
void _DNNCaffeFCN::WrapBatchInputLayer(
		std::vector<std::vector<cv::Mat> > *input_batch)
{
	Blob<float>* input_layer = net_->input_blobs()[0];
...
void _DNNCaffeFCN::PreprocessBatch(const vector<cv::Mat> imgs,
		std::vector<std::vector<cv::Mat> >* input_batch)
{
