...
#include "_WebUIswarmList.h"

namespace kai
{

	_WebUIswarmList::_WebUIswarmList()
	{
	}
	...
	bool _WebUIswarmList::init(void *pKiss)
	{
	...
	bool _WebUIswarmList::link(void)
	{
	...
	bool _WebUIswarmList::start(void)
	{
		NULL_F(m_pT);
		NULL_F(m_pTui);
	...
	int _WebUIswarmList::check(void)
	{
		NULL__(m_pSC, -1);
	...
	void _WebUIswarmList::update(void)
	{
		while (m_pT->bRun())
		{
			m_pT->autoFPSfrom();
	...
	void _WebUIswarmList::updateVehicles(void)
	{
		string h = "";
	...
	void _WebUIswarmList::show(void)
	{
		m_wd = webui_new_window();
		g_cbWebUIswarmList[g_nWebUIswarmList].m_wd = m_wd;
		g_cbWebUIswarmList[g_nWebUIswarmList].m_pI = this;
		g_nWebUIswarmList++;
	...
	void _WebUIswarmList::btn(webui_event_t *e)
	{
		IF_(check()<0);
