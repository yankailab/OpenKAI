...
static int decode_rxmraw(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_rxmraw: len=%d\n",raw->len);
    ...
    if (raw->len<12+24*nsat) {
        rtk_trace(2,"ubx rxmraw length error: len=%d nsat=%d\n",raw->len,nsat);
        return -1;
    }
    tow =U4(p  );
    week=U2(p+4);
    ...
    if (week==0) {
        rtk_trace(3,"ubx rxmraw week=0 error: len=%d nsat=%d\n",raw->len,nsat);
        return 0;
    }
    /* time tag adjustment */
    if (tadj>0.0) {
    ...
    for (i=0,p+=8;i<nsat&&i<MAXOBS;i++,p+=24) {
        ...
        if (!(sat=satno(MINPRNSBS<=prn?SYS_SBS:SYS_GPS,prn))) {
            rtk_trace(2,"ubx rxmraw sat number error: prn=%d\n",prn);
            continue;
        }
...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_rxmrawx: len=%d\n",raw->len);
    
    nsat=U1(p+11);
    if (raw->len<24+32*nsat) {
        rtk_trace(2,"ubx rxmrawx length error: len=%d nsat=%d\n",raw->len,nsat);
        return -1;
    }
    tow=R8(p);
    week=U2(p+8);
    ...
    if (week==0) {
        rtk_trace(3,"ubx rxmrawx week=0 error: len=%d nsat=%d\n",raw->len,nsat);
        return 0;
    }
    if (raw->outtype) {
    ...
    for (i=0,p+=16;i<nsat&&i<MAXOBS;i++,p+=32) {
        ...
        if (!(sys=ubx_sys(U1(p+20)))) {
            rtk_trace(2,"ubx rxmrawx: system error\n");
            continue;
        }
        prn=U1(p+21)+(sys==SYS_QZS?192:0);
        if (!(sat=satno(sys,prn))) {
            rtk_trace(2,"ubx rxmrawx sat number error: sys=%2d prn=%2d\n",sys,prn);
            continue;
        }
...
static int save_subfrm(int sat, raw_t *raw)
{
    ...
    rtk_trace(4,"save_subfrm: sat=%2d id=%d\n",sat,id);
...
static int decode_rxmsfrb(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_rxmsfrb: len=%d\n",raw->len);
    
    if (raw->outtype) {
        sprintf(raw->msgtype,"UBX RXM-SFRB  (%4d): prn=%2d",raw->len,U1(p+1));
    }
    if (raw->len<42) {
        rtk_trace(2,"ubx rxmsfrb length error: len=%d\n",raw->len);
        return -1;
    }
    prn=U1(p+1);
    if (!(sat=satno(MINPRNSBS<=prn?SYS_SBS:SYS_GPS,prn))) {
        rtk_trace(2,"ubx rxmsfrb satellite number error: prn=%d\n",prn);
        return -1;
    }
    sys=satsys(sat,&prn);
...
static int decode_navsol(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_navsol: len=%d\n",raw->len);
...
static int decode_navtime(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_navtime: len=%d\n",raw->len);
...
static int decode_trkmeas(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_trkmeas: len=%d\n",raw->len);
    ...
    if (raw->len<112+nch*56) {
        rtk_trace(2,"decode_trkmeas: length error len=%d nch=%2d\n",raw->len,nch);
        return -1;
    }
    ...
    for (i=0,p=raw->buff+110;i<nch;i++,p+=56) {
        ...
        if (!(sys=ubx_sys(U1(p+4)))) {
            rtk_trace(2,"ubx trkmeas: system error\n");
            continue;
        }
        prn=U1(p+5)+(sys==SYS_QZS?192:0);
        if (!(sat=satno(sys,prn))) {
            rtk_trace(2,"ubx trkmeas sat number error: sys=%2d prn=%2d\n",sys,prn);
            continue;
        }
        /* transmission time */
...
#if 0 /* for debug */
        rtk_trace(2,"[%2d] qi=%d sys=%d prn=%3d frq=%2d flag=%02X ?=%02X %02X "
...
static int decode_trkd5(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_trkd5: len=%d\n",raw->len);
    ...
    for (i=0,p=raw->buff+off;p-raw->buff<raw->len-2;i++,p+=len) {
        ...
        if (type==6) {
            if (!(sys=ubx_sys(U1(p+56)))) {
                rtk_trace(2,"ubx trkd5: system error\n");
                continue;
            }
        ...
        if (!(sat=satno(sys,prn))) {
            rtk_trace(2,"ubx trkd5 sat number error: sys=%2d prn=%2d\n",sys,prn);
            continue;
        }
        /* transmission time */
...
#if 0 /* for debug */
        rtk_trace(2,"[%2d] qi=%d sys=%d prn=%3d frq=%2d flag=%02X ts=%1.3f "
...
static int decode_nav(raw_t *raw, int sat, int off)
{
    ...
    if (raw->len<48+off) {
        rtk_trace(2,"ubx rawsfrbx length error: sat=%d len=%d\n",sat,raw->len);
        return -1;
    }
    ...
    id=(words[1]>>2)&7;
    if (id<1||5<id) {
        rtk_trace(2,"ubx rawsfrbx subfrm id error: sat=%2d\n",sat);
        return -1;
    }
    for (i=0;i<10;i++) {
...
static int decode_enav(raw_t *raw, int sat, int off)
{
    ...
    /* test even-odd parts */
    if (part1!=0||part2!=1) {
        rtk_trace(2,"ubx rawsfrbx gal page even/odd error: sat=%2d\n",sat);
        return -1;
    }
    ...
    if (rtk_crc24q(crc_buff,25)!=getbitu(buff+16,82,24)) {
        rtk_trace(2,"ubx rawsfrbx gal page crc error: sat=%2d\n",sat);
        return -1;
    }
    ...
    if (eph.sat!=sat) {
        rtk_trace(2,"ubx rawsfrbx gal svid error: sat=%2d %2d\n",sat,eph.sat);
        return -1;
    }
    if (!strstr(raw->opt,"-EPHALL")) {
...
static int decode_gnav(raw_t *raw, int sat, int off, int frq)
{
    ...
    if (raw->len<24+off) {
        rtk_trace(2,"ubx rawsfrbx gnav length error: len=%d\n",raw->len);
        return -1;
    }
    ...
    if (!test_glostr(buff)) {
        rtk_trace(2,"ubx rawsfrbx glo string hamming error: sat=%2d\n",sat);
        return -1;
    }
    m=getbitu(buff,1,4);
    if (m<1||15<m) {
        rtk_trace(2,"ubx rawsfrbx glo string no error: sat=%2d\n",sat);
        return -1;
    }
...
static int decode_snav(raw_t *raw, int sat, int off)
{
    ...
    if (raw->len<40+off) {
        rtk_trace(2,"ubx rawsfrbx snav length error: len=%d\n",raw->len);
        return -1;
    }
...
static int decode_rxmsfrbx(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_rxmsfrbx: len=%d\n",raw->len);
    ...
    if (!(sys=ubx_sys(U1(p)))) {
        rtk_trace(2,"ubx rxmsfrbx sys id error: sys=%d\n",U1(p));
        return -1;
    }
    prn=U1(p+1)+(sys==SYS_QZS?192:0);
    if (!(sat=satno(sys,prn))) {
        rtk_trace(2,"ubx rxmsfrbx sat number error: sys=%d prn=%d\n",sys,prn);
        return -1;
    }
    switch (sys) {
...
static int decode_trksfrbx(raw_t *raw)
{
    ...
    rtk_trace(4,"decode_trksfrbx: len=%d\n",raw->len);
    ...
    if (!(sys=ubx_sys(U1(p+1)))) {
        rtk_trace(2,"ubx trksfrbx sys id error: sys=%d\n",U1(p+1));
        return -1;
    }
    prn=U1(p+2)+(sys==SYS_QZS?192:0);
    if (!(sat=satno(sys,prn))) {
        rtk_trace(2,"ubx trksfrbx sat number error: sys=%d prn=%d\n",sys,prn);
        return -1;
    }
    switch (sys) {
...
static int decode_ubx(raw_t *raw)
{
    ...
    rtk_trace(3,"decode_ubx: type=%04x len=%d\n",type,raw->len);
    
    /* checksum */
    if (!checksum(raw->buff,raw->len)) {
        rtk_trace(2,"ubx checksum error: type=%04x len=%d\n",type,raw->len);
        return -1;
    }
    switch (type) {
...
{
    rtk_trace(5,"input_ubx: data=%02x\n",data);
    ...
    if (raw->nbyte==6) {
        if ((raw->len=U2(raw->buff+4)+8)>MAXRAWLEN) {
            rtk_trace(2,"ubx length error: len=%d\n",raw->len);
            raw->nbyte=0;
            return -1;
        }
    }
