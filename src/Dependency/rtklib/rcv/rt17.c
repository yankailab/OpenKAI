...
extern int input_rt17(raw_t *raw, unsigned char data)
{
    ...
    */
    ...
    {
        rtk_trace( 2, "RT17: Prospective packet did not end with an "
    ...
    */
    ...
    {
        rtk_trace(2, "RT17: Packet checksum failure. Packet discarded.\n");
        clear_packet_buffer(raw);
        return(0);
    }
    ...
    if (raw->pbuff[2] == RAWDATA)
    {
        ...
        */
        ...
        {       
            if (page != 1)
            {
                rtk_trace( 2, "RT17: First RAWDATA packet is not page #1. "
                          "Packet discarded.\n" );
                clear_packet_buffer(raw);
                return (0);
            }
        ...
        {
            rtk_trace( 2, "RT17: RAWDATA packet sequence number mismatch or page "
        ...
        */
        ...
        {
            rtk_trace( 2, "RT17: Buffer would overflow. "
    ...
    if (raw->pbuff[2] == GENOUT)
    {
        ...
        */
        ...
        {       
            if (page != 0)
            {
                rtk_trace( 3, "RT17: First GENOUT packet is not page #0. "
                          "Packet discarded.\n" );
                clear_packet_buffer(raw);
                return (0);
            }
        ...
        {
            rtk_trace( 2, "RT17: GENOUT packet sequence number mismatch or page "
    ...
    */
    rtk_trace( 2, "RT17: Packet is not GENOUT, RAWDATA or RETSVDATA. "
              "Packet discarded.\n" ); 
    clear_packet_buffer(raw);
    return (0);
}
...
static void decode_genout(raw_t *raw, int endian)
{
    ...
    */
    ...
    while (input_len)
    {
        ...
        rtk_trace( 4, "RT17: Packet type=0X40 (GENOUT), GSOF record type=%d (%s), "
        ...
        */
        ...
        case 41:
            ...
            break;
        default:
            rtk_trace(4, "RT17: GSOF message not processed.\n");    
        }
...
static int decode_gps_ephemeris(raw_t *raw, int e)
{
    ...
    rtk_trace(4, "RT17: decode_gps_ephemeris, length=%d\n", raw->plen);

    if (raw->plen < 182)
    {
        rtk_trace( 2, "RT17: RETSVDATA packet length %d < 182 bytes. "
    ...
    {
        rtk_trace(2, "RT17: GPS ephemeris satellite number error, PRN=%d.\n", prn);
        return (-1);
    }
    ...
    */
    ...
    rtk_trace( 4, "RT17: decode_gps_ephemeris, SAT=%d, IODC=%d, IODE=%d.\n",
...
static void decode_gsof_1(raw_t *raw, unsigned char *p, int endian)
{
    ...
    if (p[1] < 7)
       rtk_trace( 2, "RT17: GSOF Position Time message "
...
{
    if (p[1] < 10)
        rtk_trace( 2, "RT17: GSOF Current Time message "
...
{
    if (p[1] < 7)
       rtk_trace( 2, "RT17: GSOF Position Time UTC message "
                 "record length %d < 7 bytes. Record discarded.\n", p[1] );
    else
        set_week(raw, I2(p+6, endian), (double) I4(p+2, endian));
}
...
{
    if (p[1] < 7)
        rtk_trace( 2, "RT17: GSOF Base Position and Quality Indicator message "
                   "record length %d < 7 bytes. Record discarded.\n", p[1] );
    else 
        set_week(raw, I2(p+6, endian), (double) I4(p+2, endian));
}
...
static int decode_ion_utc_data(raw_t *raw, int e)
{
    ...
    rtk_trace(4, "RT17: decode_ion_utc_data, length=%d.\n", raw->plen);

    if (raw->plen < 129)
    {
        rtk_trace( 2, "RT17: RETSVDATA packet length %d < 129 bytes. "
...
static int decode_rawdata(raw_t *raw, int endian)
{
    ...
    rtk_trace( 3, "RT17: Packet type=0X57 (RAWDATA), recordtype=%d (%s), "
    ...
    */
    ...
    case 7:
        ...
        break;
    default:
        rtk_trace(3, "RT17: Packet not processed.\n");      
    }
...
static int decode_retsvdata(raw_t *raw, int endian)
{
    ...
    rtk_trace( 3, "RT17: packet type=0X55 (RETSVDATA), subtype=%d (%s), "
...
static int decode_type_17(raw_t *raw, unsigned int rif, int e)
{
    ...
    */   
    ...
    for (i=0; (i < nsat) && (i < MAXOBS); i++)
    {
        ...
        if (!(sat=satno(SYS_GPS, prn)))
        {
            rtk_trace(2, "RT17: Satellite number error, PRN=%d.\n", prn);
            continue;
        }
...
static int decode_type_29(raw_t *raw, int endian)
{
    ...
    if (*p < 7)
       rtk_trace( 2, "RT17: Enhanced Position record block #1 length %d < 7 bytes. "
                 "Record discarded.\n", *p );
    else
...
static int get_week(raw_t *raw, double receive_time)
{
    ...
    if (raw->flag & M_WEEK_OPTION)
    {
        ...
        {
            rtk_trace( 2, "RT17: GPS WEEK rolled over from %d to %d.\n",
                   raw->week, raw->week + 1 );
    ...
    else if (!(raw->flag & M_WEEK_SCAN))
    {
        ...
        if (opt)
        {
            if (!sscanf(opt+6, "%d", &week) || (week <= 0))
                rtk_trace(2, "RT17: Invalid -WEEK=n receiver option value.\n");
            else
            {
                raw->week = week;
                raw->flag |= M_WEEK_OPTION;

                rtk_trace( 2, "RT17: Initial GPS WEEK explicitly set to %d "
                          "by user.\n", week );
            }
        }
    }
    ...
    if (week == 0)
    {
        ...
        rtk_trace( 2, "RT17: Initial GPS WEEK number unknown; "
        ...
        if (raw->week)
        {
            if (week != raw->week)
            {
                if (week == (raw->week + 1))
                    ...
                    rtk_trace(2, "RT17: GPS WEEK changed from %d to %d.\n", raw->week, week);
            }
        }
        else
            rtk_trace(2, "RT17: GPS WEEK initially set to %d.\n", week);
    ...
    *rif = p_in[7];
    ...
    while (length_in_total > 0)
    {
        if ((unsigned int)p_in[7] != *rif)
           rtk_trace( 2, "RT17: Inconsistent Record Interpretation "
