...
static void testeclipse(const obsd_t *obs, int n, const nav_t *nav, double *rs)
{
    ...
    for (i=0;i<n;i++) {
        ...
        rtk_trace(2,"eclipsing sat excluded %s sat=%2d\n",time_str(obs[0].time,0),
              obs[i].sat);
                    ...
                    double *meas, double *var, int *brk)
{
    ...
    if (!corr_ion(obs->time,nav,obs->sat,pos,azel,opt->ionoopt,&ion,&vari,brk)) {
        ...
        rtk_trace(2,"iono correction error: time=%s sat=%2d ionoopt=%d\n",
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=k=0;i<n&&i<MAXOBS;i++) {
        ...
        if (brk) {
            ...
            rtk_trace(2,"%s: sat=%2d correction break\n",time_str(obs[i].time,0),sat);
        }
                   ...
                   double *H, double *R, double *azel)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        rtk_trace(5,"sat=%2d azel=%6.1f %5.1f dtrp=%.3f dantr=%6.3f %6.3f dants=%6.3f %6.3f phw=%6.3f\n",
...
#endif
                rtk_trace(2,"ppp outlier rejected %s sat=%2d type=%d v=%.3f\n",
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<rtk->opt.niter;i++) {
        ...
        if ((info=rtk_filter(xp,Pp,H,v,R,rtk->nx,nv))) {
            rtk_trace(2,"ppp filter error %s info=%d\n",time_str(rtk->sol.time,0),
                  info);
            break;
        }
