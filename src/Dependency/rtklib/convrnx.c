...
static int input_strfile(strfile_t *str)
{
    ...
    rtk_trace(4,"input_strfile: time=%s type=%d sat=%2d\n",time_str(str->time,3),
          type,str->sat);
    return type;
}
/* open stream file ----------------------------------------------------------*/
...
{
    rtk_trace(3,"open_strfile: file=%s\n",file);
                           ...
                           const unsigned char *types, int sys, rnxopt_t *opt)
{
    ...
    rtk_trace(3,"setopt_obstype: sys=%d\n",sys);
                        ...
                        gtime_t *time)
{
    ...
    rtk_trace(3,"scan_obstype: file=%s, opt=%s\n",file,opt);
  ...
  //      if ((abort=//showmsg(msg))) break;
    ...
    if (abort) {
        rtk_trace(2,"aborted in scan\n");
        return 0;
    }
    for (i=0;i<6;i++) for (j=0;j<n[i];j++) {
        rtk_trace(2,"scan_obstype: sys=%d code=%s type=%d\n",i,code2obs(codes[i][j],NULL),types[i][j]);
    }
    for (i=0;i<6;i++) {
        ...
        for (j=0;j<n[i];j++) {
            rtk_trace(3,"scan_obstype: sys=%d code=%s\n",i,code2obs(codes[i][j],NULL));
        }
    }
    return 1;
}
...
static void set_obstype(int format, rnxopt_t *opt)
{
    ...
    rtk_trace(3,"set_obstype: format=%d\n",format);
...
static void convsbs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n)
{
    ...
    if      (MINPRNSBS<=prn&&prn<=MAXPRNSBS) sys=SYS_SBS;
        ...
        rtk_trace(2,"sbas message satellite error: prn=%d\n",prn);
        return;
    }
...
static void setapppos(strfile_t *str, rnxopt_t *opt)
{
    ...
    if (!pntpos(str->obs->data,str->obs->n,str->nav,&prcopt,&sol,NULL,NULL,
                msg)) {
        rtk_trace(2,"point position error (%s)\n",msg);
        return;
    }
                     ...
                     char **ofile)
{
    ...
    rtk_trace(3,"convrnx_s: sess=%d format=%d file=%s ofile=%s %s %s %s %s %s %s\n",
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile)
{
    ...
    rtk_trace(3,"convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s\n",
