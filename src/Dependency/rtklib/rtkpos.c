...
extern int rtkopenstat(const char *file, int level)
{
    ...
    rtk_trace(3,"rtkopenstat: file=%s level=%d\n",file,level);
    ...
    if (!(fp_stat=fopen(path,"w"))) {
        rtk_trace(1,"rtkopenstat: file open error path=%s\n",path);
        return 0;
    }
...
static void swapsolstat(void)
{
    ...
    if (!(fp_stat=fopen(path,"w"))) {
        rtk_trace(2,"swapsolstat: file open error path=%s\n",path);
        return;
    }
    rtk_trace(3,"swapsolstat: path=%s\n",path);
}
                  ...
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {
        if      (obs[i].sat<obs[j].sat) j--;
            ...
            rtk_trace(4,"(%2d) sat=%3d iu=%2d ir=%2d\n",k-1,obs[i].sat,i,j);
        }
    }
    return k;
}
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    if (var>VAR_POS) {
        ...
        for (i=3;i<6;i++) initx(rtk,rtk->sol.rr[i],VAR_VEL,i);
        for (i=6;i<9;i++) initx(rtk,1E-6,VAR_ACC,i);
        rtk_trace(2,"reset rtk position due to large variance: var=%.3f\n",var);
        return;
    }
...
static void udion(rtk_t *rtk, double tt, double bl, const int *sat, int ns)
{
    ...
    rtk_trace(3,"udion   : tt=%.1f bl=%.0f ns=%d\n",tt,bl,ns);
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv)
{
    ...
    rtk_trace(3,"detslp_ll: i=%d rcv=%d\n",i,rcv);
                           ...
                           const nav_t *nav)
{
    ...
    rtk_trace(3,"detslp_gf_L1L2: i=%d j=%d\n",i,j);
                           ...
                           const nav_t *nav)
{
    ...
    rtk_trace(3,"detslp_gf_L1L5: i=%d j=%d\n",i,j);
                       ...
                       const nav_t *nav)
{
    ...
    rtk_trace(3,"detslp_dop: i=%d rcv=%d\n",i,rcv);
    ...
    for (f=0;f<nf;f++) {
        ...
        for (i=1;i<=MAXSAT;i++) {
            ...
            if (rtk->opt.modear==ARMODE_INST&&rtk->x[IB(i,f,&rtk->opt)]!=0.0) {
                ...
                initx(rtk,0.0,0.0,IB(i,f,&rtk->opt));
                rtk_trace(3,"udbias : obs outage counter overflow (sat=%3d L%d n=%d)\n",
                 ...
                 double *e, double *azel)
{
    ...
    rtk_trace(4,"rr_=%.3f %.3f %.3f\n",rr_[0],rr_[1],rr_[2]);
    rtk_trace(4,"pos=%.9f %.9f %.3f\n",pos[0]*R2D,pos[1]*R2D,pos[2]);
    for (i=0;i<n;i++) {
        rtk_trace(4,"sat=%2d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
                   ...
                   double *H, double *Ri, double *Rj, int index)
{
    ...
    if (var>thres*thres*bb*bb) {
        rtk_trace(3,"constbl : equation nonlinear (bb=%.3f var=%.3f)\n",bb,var);
        return 0;
    }
    ...
    rtk_trace(4,"baseline len   v=%13.3f R=%8.6f %8.6f\n",v[index],Ri[index],Rj[index]);
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    rtk_trace(3,"ddres   : dt=%.1f nx=%d ns=%d\n",dt,rtk->nx,ns);
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (j=0;j<ns;j++) {
            ...
            rtk_trace(4,"sat=%3d-%3d %s%d v=%13.3f R=%8.6f %8.6f\n",sat[i],
...
static void holdamb(rtk_t *rtk, const double *xa)
{
    ...
    if (nv>0) {
        ...
        if ((info=rtk_filter(rtk->x,rtk->P,H,v,R,rtk->nx,nv))) {
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    rtk_trace(3,"resamb_LAMBDA : nx=%d\n",nx);
    ...
    rtk_trace(4,"N(0)="); tracemat(4,y+na,1,nb,10,3);
    
    /* lambda/mlambda integer least-square estimation */
    if (!(info=lambda(nb,2,y+na,Qb,b,s))) {
        
        rtk_trace(4,"N(1)="); tracemat(4,b   ,1,nb,10,3);
        rtk_trace(4,"N(2)="); tracemat(4,b+nb,1,nb,10,3);
        ...
        if (s[0]<=0.0||s[1]/s[0]>=opt->thresar[0]) {
            ...
            if (!matinv(Qb,nb)) {
                ...
                rtk_trace(3,"resamb : validation ok (nb=%d ratio=%.2f s=%.2f/%.2f)\n",
                  ...
                  int nv, double thres)
{
    ...
    rtk_trace(3,"valpos  : nv=%d thres=%.1f\n",nv,thres);
    ...
    if (stat&&nv>NP(opt)) {
        ...
        if (vv>chisqr[nv-NP(opt)-1]) {
            ...
            rtk_trace(3,"valpos : validation ok (%s nv=%d np=%d vv=%.2f cs=%.2f)\n",
                  ...
                  const nav_t *nav)
{
    ...
    rtk_trace(3,"relpos  : nx=%d nu=%d nr=%d\n",rtk->nx,nu,nr);
    ...
    rtk_trace(4,"x(0)="); tracemat(4,rtk->x,1,NR(opt),13,4);
    ...
    for (i=0;i<niter;i++) {
        ...
        rtk_trace(4,"x(%d)=",i+1); tracemat(4,xp,1,NR(opt),13,4);
    }
