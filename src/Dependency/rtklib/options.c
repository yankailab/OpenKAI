...
extern int loadopts(const char *file, opt_t *opts)
{
    ...
    rtk_trace(3,"loadopts: file=%s\n",file);
    
    if (!(fp=fopen(file,"r"))) {
        rtk_trace(1,"loadopts: options file open error (%s)\n",file);
        return 0;
    }
                    ...
                    const opt_t *opts)
{
    ...
    rtk_trace(3,"saveopts: file=%s mode=%s\n",file,mode);
    
    if (!(fp=fopen(file,mode))) {
        rtk_trace(1,"saveopts: options file open error (%s)\n",file);
        return 0;
    }
