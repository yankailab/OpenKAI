                   ...
                   const double *NC, int n)
{
    ...
    if ((info=rtk_filter(rtk->x,rtk->P,H,v,R,rtk->nx,n))) {
        rtk_trace(1,"filter error (info=%d)\n",info);
...
static int fix_amb_ILS(rtk_t *rtk, int *sat1, int *sat2, int *NW, int n)
{
    ...
    if ((info=lambda(m,2,B1,Q,N1,s))) {
        rtk_trace(2,"lambda error: info=%d\n",info);
        return 0;
    }
    if (s[0]<=0.0) return 0;
    ...
    if (rtk->opt.thresar[0]>0.0&&rtk->sol.ratio<rtk->opt.thresar[0]) {
        rtk_trace(2,"varidation error: n=%2d ratio=%8.3f\n",m,rtk->sol.ratio);
        return 0;
    }
    rtk_trace(2,"varidation ok: %s n=%2d ratio=%8.3f\n",time_str(rtk->sol.time,0),m,
          rtk->sol.ratio);
