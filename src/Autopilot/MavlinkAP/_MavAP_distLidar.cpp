#include "_MavAP_distLidar.h"

namespace kai
{

	_MavAP_distLidar::_MavAP_distLidar()
	{
		m_pAP = nullptr;
		m_pDS = nullptr;
		m_nSection = 0;
	...
	int _MavAP_distLidar::init(void *pKiss)
	{
	...
	void _MavAP_distLidar::update(void)
	{
		updateMavlink();
	}

	void _MavAP_distLidar::updateMavlink(void)
	{
		NULL_(m_pAP);
		NULL_(m_pAP->m_pMav);
	...
	void _MavAP_distLidar::draw(void *pFrame)
	{
#ifdef USE_OPENCV
		NULL_(pFrame);
