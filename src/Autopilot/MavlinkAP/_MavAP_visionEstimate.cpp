#include "_MavAP_visionEstimate.h"

namespace kai
{

	_MavAP_visionEstimate::_MavAP_visionEstimate()
	{
		m_pAP = nullptr;
		m_pNav = nullptr;
	...
	int _MavAP_visionEstimate::init(void *pKiss)
	{
	...
	int _MavAP_visionEstimate::link(void)
	{
	...
	int _MavAP_visionEstimate::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_visionEstimate::check(void)
	{
	...
	void _MavAP_visionEstimate::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	bool _MavAP_visionEstimate::updateVisionEstimate(void)
	{
	...
	bool _MavAP_visionEstimate::bNaN(void)
	{
		IF_F(check() != OK_OK);
	...
	void _MavAP_visionEstimate::updateResetCounter(void)
	{
		vFloat3 vT = m_pNav->t();
		vFloat3 vV = m_pNav->v();
	...
	void _MavAP_visionEstimate::sendPosEstimate(void)
	{
	...
	void _MavAP_visionEstimate::sendSpeedEstimate(void)
	{
		vFloat3 vV = m_pNav->v();
	...
	void _MavAP_visionEstimate::console(void *pConsole)
	{
		NULL_(pConsole);
