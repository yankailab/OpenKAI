...
namespace kai
{
	...
	int _MavAP_move::init(void *pKiss)
	{
	...
	int _MavAP_move::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_move::check(void)
	{
	...
	void _MavAP_move::setHdg(float y, float r, bool bYaw, bool bYawRate, uint8_t frame)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_move::setVlocal(const vFloat4 &vSpd, bool bYaw, bool bYawRate, uint8_t frame)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_move::setPlocal(const vFloat4 &vP, bool bYaw, bool bYawRate, uint8_t frame)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_move::setPglobal(const vDouble4 &vP, bool bYaw, bool bYawRate, uint8_t frame)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_move::doReposition(const vDouble4 &vP,
								float speed,
								float radius,
								uint8_t frame)
	{
	...
	void _MavAP_move::console(void *pConsole)
	{
		NULL_(pConsole);
