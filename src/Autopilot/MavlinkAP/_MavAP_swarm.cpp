#include "_MavAP_swarm.h"

namespace kai
{

	_MavAP_swarm::_MavAP_swarm()
	{
		m_pAP = nullptr;
		m_pXb = nullptr;
		m_pSwarm = nullptr;
	...
	int _MavAP_swarm::init(void *pKiss)
	{
		CHECK_(this->_MavAP_move::init(pKiss));
		Kiss *pK = (Kiss *)pKiss;
	...
	int _MavAP_swarm::link(void)
	{
		CHECK_(this->_MavAP_move::link());
	...
	int _MavAP_swarm::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_swarm::check(void)
	{
		...
		return this->_MavAP_move::check();
	}

	void _MavAP_swarm::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	void _MavAP_swarm::updateState(void)
	{
		IF_(check() != OK_OK);
	...
	bool _MavAP_swarm::findBeacon(void)
	{
		IF_F(check() != OK_OK);
	...
	bool _MavAP_swarm::findVisual(void)
	{
		IF_F(check() != OK_OK);
	...
	bool _MavAP_swarm::findVisualTarget(void)
	{
		IF_F(check() != OK_OK);
	...
	void _MavAP_swarm::calcMove(void)
	{
	...
	SWARM_DETECTION *_MavAP_swarm::getDetByID(uint16_t id)
	{
	...
	SWARM_DETECTION *_MavAP_swarm::getDetByDist(vDouble2 vP, double r)
	{
		for (int i = 0; i < m_vDetections.size(); i++)
		{
			SWARM_DETECTION *pD = &m_vDetections[i];
	...
	void _MavAP_swarm::gimbalDownward(void)
	{
		NULL_(m_pGio);
		IF_(!m_pGio->bOpen());
	...
	char _MavAP_swarm::calcCRC(const char *cmd, uint8_t len)
	{
		char crc = 0;
		for (int i = 0; i < len; i++)
		{
	...
	void _MavAP_swarm::sendDetectionHB(void)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_swarm::sendGCupdate(void)
	{
		SWMSG_GC_UPDATE m;
		m.m_srcID = m_myID;
		m.m_dstID = 0;
	...
	void _MavAP_swarm::onRecvMsg(const XBframe_receivePacket &d)
	{
	...
	void _MavAP_swarm::handleMsgSetState(const SWMSG_CMD_SETSTATE &m)
	{
		IF_(check() != OK_OK);
