#include "_MavAP_land.h"

namespace kai
{

	_MavAP_land::_MavAP_land()
	{
		m_pDS = nullptr;
		m_vDSrange.clear();
	...
	int _MavAP_land::init(void *pKiss)
	{
		CHECK_(this->_MavAP_follow::init(pKiss));
		Kiss *pK = (Kiss *)pKiss;
	...
	int _MavAP_land::link(void)
	{
		CHECK_(this->_MavAP_follow::link());
	...
	int _MavAP_land::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_land::check(void)
	{
		NULL__(m_pDS, OK_ERR_NULLPTR);

		return this->_MavAP_follow::check();
	}

	void _MavAP_land::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	bool _MavAP_land::bComplete(void)
	{
		IF_F(check() != OK_OK);
		IF_F(!m_bTarget);
	...
	void _MavAP_land::updateMove(void)
	{
		IF_(check() != OK_OK);
	...
	AP_LAND_TAG *_MavAP_land::getTag(int id)
	{
	...
	void _MavAP_land::updatePID(void)
	{
