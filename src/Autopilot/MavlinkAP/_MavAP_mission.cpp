#include "_MavAP_mission.h"

namespace kai
{

	_MavAP_mission::_MavAP_mission()
	{
		m_pAP = nullptr;
		m_pAPmove = nullptr;
		m_iMission = 0;
	...
	int _MavAP_mission::init(void *pKiss)
	{
	...
	int _MavAP_mission::link(void)
	{
		...
		n = "";
		pK->v("_MavAP_move", &n);
		m_pAPmove = (_MavAP_move *)(pK->findModule(n));
		NULL__(m_pAPmove, OK_ERR_NOT_FOUND);

		return OK_OK;
	}

	int _MavAP_mission::start(void)
	{
	...
	void _MavAP_mission::updateW(void)
	{
		while (m_pT->bAlive())
		{
			if (!m_pIO)
	...
	void _MavAP_mission::updateMission(void)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_mission::updateR(void)
	{
		while (m_pTr->bAlive())
		{
			m_pTr->autoFPSfrom();
	...
	void _MavAP_mission::handleMsg(string &str)
	{
		value json;
	...
	void _MavAP_mission::heartbeat(picojson::object &o)
	{
		IF_(check() != OK_OK);
	}

	void _MavAP_mission::stat(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::missionUpdate(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::missionStart(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::missionPause(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::missionResume(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::missionStop(picojson::object &o)
	{
		IF_(check() != OK_OK);
		IF_(!o["id"].is<double>());
	...
	void _MavAP_mission::console(void *pConsole)
	{
		NULL_(pConsole);
