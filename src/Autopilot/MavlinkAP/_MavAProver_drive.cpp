#include "_MavAProver_drive.h"

namespace kai
{

	_MavAProver_drive::_MavAProver_drive()
	{
		m_pAP = nullptr;
		m_pD = nullptr;
	...
	int _MavAProver_drive::init(void *pKiss)
	{
	...
	int _MavAProver_drive::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAProver_drive::check(void)
	{
	...
	void _MavAProver_drive::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	void _MavAProver_drive::onPause(void)
	{
	...
	bool _MavAProver_drive::updateDrive(void)
	{
		IF_F(check() != OK_OK);
	...
	void _MavAProver_drive::setYawMode(bool bRelative)
	{
		if (bRelative)
			m_yawMode = 1.0;
		else
	...
	void _MavAProver_drive::console(void *pConsole)
	{
		NULL_(pConsole);
