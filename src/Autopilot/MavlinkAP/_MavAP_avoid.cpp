#include "_MavAP_avoid.h"

namespace kai
{

	_MavAP_avoid::_MavAP_avoid()
	{
		m_pAP = nullptr;
		m_pDet = nullptr;
		m_pMavlink = nullptr;
	...
	int _MavAP_avoid::init(void *pKiss)
	{
	...
	int _MavAP_avoid::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_avoid::check(void)
	{
	...
	void _MavAP_avoid::updateTarget(void)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_avoid::console(void *pConsole)
	{
		NULL_(pConsole);
	...
	void _MavAP_avoid::draw(void *pFrame)
	{
		NULL_(pFrame);
