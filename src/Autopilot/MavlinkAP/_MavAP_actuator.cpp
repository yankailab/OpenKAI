#include "_MavAP_actuator.h"

namespace kai
{

	_MavAP_actuator::_MavAP_actuator()
	{
		m_pAP = nullptr;
		m_pAB1 = nullptr;
		m_pAB2 = nullptr;
	...
	int _MavAP_actuator::init(void *pKiss)
	{
	...
	int _MavAP_actuator::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_actuator::check(void)
	{
	...
	void _MavAP_actuator::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	void _MavAP_actuator::updateActuator(void)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_actuator::console(void *pConsole)
	{
		NULL_(pConsole);
		IF_(check() != OK_OK);
