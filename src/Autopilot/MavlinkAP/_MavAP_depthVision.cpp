#include "_MavAP_depthVision.h"

namespace kai
{

	_MavAP_depthVision::_MavAP_depthVision()
	{
		m_pAP = nullptr;
		m_pDV = nullptr;
		m_nROI = 0;
	...
	int _MavAP_depthVision::init(void *pKiss)
	{
	...
	void _MavAP_depthVision::update(void)
	{
		NULL_(m_pAP);
		NULL_(m_pAP->m_pMav);
	...
	void _MavAP_depthVision::draw(void *pFrame)
	{
		NULL_(pFrame);
