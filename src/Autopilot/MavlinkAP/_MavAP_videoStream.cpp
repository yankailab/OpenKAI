#include "_MavAP_videoStream.h"

namespace kai
{

	_MavAP_videoStream::_MavAP_videoStream()
	{
		m_pAP = NULL;
		m_pCurl = NULL;
		m_fName = "";
	...
	int _MavAP_videoStream::init(void *pKiss)
	{
	...
	int _MavAP_videoStream::link(void)
	{
	...
	int _MavAP_videoStream::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_videoStream::check(void)
	{
	...
	void _MavAP_videoStream::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	void _MavAP_videoStream::updateStream(void)
	{
		IF_(check() != OK_OK);
	...
	bool _MavAP_videoStream::openStream(void)
	{
		IF_F(check() != OK_OK);
		// IF_F(m_pFvid);
		IF_F(m_gstPID);
	...
	void _MavAP_videoStream::closeStream(void)
	{
		IF_(!m_gstPID);
	...
	void _MavAP_videoStream::console(void *pConsole)
	{
		NULL_(pConsole);
