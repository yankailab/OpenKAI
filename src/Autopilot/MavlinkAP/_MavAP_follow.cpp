#include "_MavAP_follow.h"

namespace kai
{

	_MavAP_follow::_MavAP_follow()
	{
		m_pU = nullptr;
		m_pTracker = nullptr;
		m_iClass = -1;
	...
	int _MavAP_follow::init(void *pKiss)
	{
	...
	int _MavAP_follow::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_follow::check(void)
	{
	...
	void _MavAP_follow::update(void)
	{
		while (m_pT->bAlive())
		{
			m_pT->autoFPSfrom();
	...
	void _MavAP_follow::onPause(void)
	{
		clearPID();
		m_bTarget = false;
		if (m_pTracker)
	...
	bool _MavAP_follow::updateTarget(void)
	{
		IF_F(check() != OK_OK);
	...
	bool _MavAP_follow::findTarget(void)
	{
		IF_F(check() != OK_OK);
	...
	void _MavAP_follow::updatePID(void)
	{
	...
	void _MavAP_follow::clearPID(void)
	{
		m_tLastPIDupdate = 0;
	...
	void _MavAP_follow::console(void *pConsole)
	{
		NULL_(pConsole);
		this->_MavAP_move::console(pConsole);
	...
	void _MavAP_follow::draw(void *pFrame)
	{
#ifdef USE_OPENCV
		NULL_(pFrame);
		this->_MavAP_move::draw(pFrame);
		IF_(check() != OK_OK);
