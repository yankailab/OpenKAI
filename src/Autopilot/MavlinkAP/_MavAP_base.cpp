...
namespace kai
{
	...
	int _MavAP_base::init(void *pKiss)
	{
	...
	int _MavAP_base::start(void)
	{
		NULL__(m_pT, OK_ERR_NULLPTR);
		return m_pT->start(getUpdate, this);
	}

	int _MavAP_base::check(void)
	{
	...
	void _MavAP_base::updateBase(void)
	{
		IF_(check() != OK_OK);
	...
	void _MavAP_base::setMode(uint32_t iMode)
	{
		IF_(check() != OK_OK);
	...
	int _MavAP_base::getMode(void)
	{
		return m_apMode;
	}

	string _MavAP_base::getModeName(void)
	{
	...
	void _MavAP_base::setArm(bool bArm)
	{
		IF_(check() != OK_OK);
	...
	bool _MavAP_base::bApArmed(void)
	{
		return m_bApArmed;
	}

	int _MavAP_base::getGPSfixType(void)
	{
		return m_gpsFixType;
	}

	int _MavAP_base::getGPShacc(void)
	{
		return m_gpsHacc;
	}

	vDouble3 _MavAP_base::getHomePos(void)
	{
		if (!m_bHomeSet)
		{
			vDouble3 vH;
	...
	vDouble4 _MavAP_base::getGlobalPos(void)
	{
		return m_vGlobalPos;
	}

	float _MavAP_base::getHdg(void)
	{
		return m_apHdg;
	}

	float _MavAP_base::getBattery(void)
	{
		return m_battery;
	}

	int _MavAP_base::getWPseq(void)
	{
		IF__(!m_pMav, -1);
	...
	int _MavAP_base::getWPtotal(void)
	{
		IF__(!m_pMav, -1);
	...
	void _MavAP_base::setMount(AP_MOUNT &m)
	{
		IF_(check() != OK_OK);
	...
	vFloat3 _MavAP_base::getSpeed(void)
	{
		return m_vSpeed;
	}

	vFloat3 _MavAP_base::getAttitude(void)
	{
		return m_vAtti;
	}

	void _MavAP_base::console(void *pConsole)
	{
		NULL_(pConsole);
