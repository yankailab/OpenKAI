#include "_APmav_photo.h"

namespace kai
{

	_APmav_photo::_APmav_photo()
	{
	}
	...
	bool _APmav_photo::init(const json &j)
	{
	...
	bool _APmav_photo::link(const json &j, ModuleMgr *pM)
	{
	...
	bool _APmav_photo::start(void)
	{
		NULL_F(m_pT);
		return m_pT->startThread(getUpdate, this);
	}

	bool _APmav_photo::check(void)
	{
		NULL_F(m_pAP);
	...
	void _APmav_photo::updatePhoto(void)
	{
		IF_(!check());
	...
	void _APmav_photo::console(void *pConsole)
	{
		NULL_(pConsole);
