...
namespace kai
{
	...
	void _AP_link::handleCMD(void)
	{
		...
		case APLINK_BB:
			...
			m_tBB = m_pT->getTfrom();
			LOG_I("BB=(" + f2str(m_vBB.x) + "," + f2str(m_vBB.y) + "," + f2str(m_vBB.z) + "," + f2str(m_vBB.w) + ")");
			break;
		...
		case APLINK_TARGET:
			...
			LOG_I("TargetBB=(" + f2str(m_vTargetBB.x) + "," + f2str(m_vTargetBB.y) + "," + f2str(m_vTargetBB.z) + "," + f2str(m_vTargetBB.w) + ")");
			break;
