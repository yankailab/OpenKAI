...
namespace kai
{
	...
	bool _APmav_mission::init(const json &j)
	{
	...
	bool _APmav_mission::link(const json &j, ModuleMgr *pM)
	{
	...
	void _APmav_mission::update(void)
	{
		while (m_pT->bRun())
		{
			m_pT->autoFPS();
	...
	void _APmav_mission::updateMission(void)
	{
		IF_(!check());
	...
	void _APmav_mission::clearMission(void)
	{
		m_vMissionDL.clear();
		m_mIdxDL = -1;
	}
	...
	// Upload
	void _APmav_mission::uploadMission(void)
	{
		// TODO:
	}
	...
	void _APmav_mission::CbMavRecvMissionAck(void *pMsg)
	{
		IF_(!check());
		NULL_(pMsg);
		IF_(m_mState != apMission_UL_missionRequestInt);
	...
	// Download
	void _APmav_mission::downloadMission(void)
	{
	...
	void _APmav_mission::CbMavRecvMissionCount(void *pMsg)
	{
		IF_(!check());
		NULL_(pMsg);
		IF_(m_mState != apMission_DL_missionCount);
	...
	{
		if (m_tOut.bTout(getTns()))
			m_mState = apMission_none;
	}

	void _APmav_mission::missionCurrent(void)
	{
		IF_(!check());
