...
bool _SegNet::init(string name, JSON* pJson)
{
	...
	string trainedFile;
	string labelFile;
//	CHECK_FATAL(pJson->v("SEGNET_MODEL_FILE_"+name, &modelFile));
//	CHECK_FATAL(pJson->v("SEGNET_WEIGHTS_FILE_"+name, &trainedFile));
//	CHECK_FATAL(pJson->v("SEGNET_COLOR_FILE_"+name, &labelFile));
//
//	CHECK_FATAL(pJson->v("SEGNET_CUDADEVICE_ID_"+name, &m_cudaDeviceID));
