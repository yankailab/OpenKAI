...
namespace kai
{
	...
	bool _RStracking::updateRS(void)
	{
		try
		{
			...
			m_vT[m_vAxisIdx.z()] = pose.translation.z * m_scale;
			...
			// m_vR[m_vAxisIdx.x()] = -asin(2.0 * (x * z - w * y)) * ovP;								  //pitch
			// m_vR[m_vAxisIdx.y()] = atan2(2.0 * (w * x + y * z), w * w - x * x - y * y + z * z) * ovP; //roll
			// m_vR[m_vAxisIdx.z()] = atan2(2.0 * (w * z + x * y), w * w + x * x - y * y - z * z) * ovP; //yaw
		}
		catch (const rs2::camera_disconnected_error &e)
		{
			LOG_E("Realsense tracking disconnected");
