...
#include "_ARmeasureFree.h"

namespace kai
{

	_ARmeasureFree::_ARmeasureFree()
	{
		m_pA = NULL;
		m_pR = NULL;
		m_pW = NULL;
	...
	bool _ARmeasureFree::start(void)
	{
		NULL_F(m_pT);
		return m_pT->start(getUpdate, this);
	}

	int _ARmeasureFree::check(void)
	{
		NULL__(m_pA, -1);
		NULL__(m_pW, -1);
		NULL__(m_pR, -1);
	...
	// callbacks
	void _ARmeasureFree::sOnBtnAction(void *pInst, uint32_t f)
	{
		NULL_(pInst);
		((_ARmeasureFree *)pInst)->action();
	}

	void _ARmeasureFree::sOnBtnClear(void *pInst, uint32_t f)
	{
		NULL_(pInst);
	...
	// UI handler
	void _ARmeasureFree::action(void)
	{
		IF_(!m_pA->bValid());
	...
	// UI draw
	void _ARmeasureFree::drawVertices(Mat *pM)
	{
		NULL_(pM);
		NULL_(m_pFt);
	...
	void _ARmeasureFree::drawMeasure(Mat *pM)
	{
		NULL_(pM);
		NULL_(m_pFt);
	...
	void _ARmeasureFree::draw(void* pFrame)
	{
		NULL_(pFrame);
		IF_(!bActive());
