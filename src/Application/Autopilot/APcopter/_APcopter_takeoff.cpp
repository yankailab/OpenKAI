#include "_APcopter_takeoff.h"

namespace kai
{

_APcopter_takeoff::_APcopter_takeoff()
{
	m_pAP = NULL;
	m_iWP = -1;
}
...
bool _APcopter_takeoff::init(void* pKiss)
{
...
bool _APcopter_takeoff::start(void)
{
	m_bThreadON = true;
...
int _APcopter_takeoff::check(void)
{
	NULL__(m_pAP, -1);
...
void _APcopter_takeoff::update(void)
{
	while (m_bThreadON)
	{
		this->autoFPSfrom();
...
void _APcopter_takeoff::updateMission(void)
{
	IF_(check()<0);
...
void _APcopter_takeoff::draw(void)
{
