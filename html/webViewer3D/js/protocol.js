...
export function decodeFrame(buffer) {
  ...
  if (v.getUint32(0, true) !== 0x31443357 || v.getUint32(4, true) !== 1)
  ...
  for (let i = 0; i < count; ++i) {
    ...
    if (ids.has(id) || at + 64 + nP * 16 + nL * 32 > buffer.byteLength)
    ...
    at += 64;
    objects.push({ id, nP, nL, pointSize, opacity, bounds,
      points: floats(nP * 3), pointColors: bytes(nP * 4), lines: floats(nL * 6), lineColors: bytes(nL * 8) });
  }
