...
int main(int argc, char **argv)
{
    ...
    using namespace kai::webselectableoctgrid;
    ...
    {
        const std::string hello = std::string(R"({"type":"hello","version":5,"stream":")") + name(Types[i]) +
            R"(","autoBound":true,"showGrid":true,"background":[0.035,0.045,0.065],"camera":{"eye":[0,-8,4],"target":[0,0,0],"up":[0,0,1],"type":0,"fov":70,"near":0.01,"far":1000000,"lr":[-10,10],"bt":[-10,10]},"objects":[{"id":7,"name":"Test cloud and lines","selectableGrid":false},{"id":8,"name":"Test grid","selectableGrid":true}]})";
    ...
    std::thread producer([&] {
        ...
        for (int i = 0; i < nPoints; ++i)
        {
            ...
            colors.insert(colors.end(), {uint8_t(i % 256), 200, 240});
        }
        ...
        while (running)
        {
            ...
            for (size_t i = 0; i < Types.size(); ++i)
            {
                ...
                if (Types[i] == Type::Points) kai::webselectableoctgrid::points(*frame, 7, 2, bounds, points, colors);
                else if (Types[i] == Type::Lines) lines(*frame, 7, bounds, {-1,0,0,1,0,0}, {255,0,0,255,0,0});
                else cells(*frame, 8, 1, bounds, grid);
                finish(*frame, 1);
