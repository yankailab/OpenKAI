...
viewer.configure({ objects: [{ id: 7, name: 'grid', selectableGrid: true }, { id: 8, name: 'geometry', selectableGrid: false }], background: [0, 0, 0], showGrid: false,
...
const pointFrame = { type: 'points', objects: [{ id: 8, count: 1, pointSize: 4, opacity: 1,
  bounds: [4, 0, 0, 4, 0, 0], positions: new Float32Array([4, 0, 0]), colors: new Uint8Array([255, 0, 0]) }] };
const lineFrame = { type: 'lines', objects: [{ id: 8, count: 1, pointSize: 1, opacity: 1,
  bounds: [-4, 0, 0, 0, 0, 0], positions: new Float32Array([-4, 0, 0, 0, 0, 0]), colors: new Uint8Array([0, 0, 255, 0, 0, 255]) }] };
...
check(!object.points && !object.lines, 'Grid allocated geometry resources');
...
const geometry = viewer.objects.get(8);
check(viewer.objects.size === 2 && geometry.streams.size === 2 && !geometry.boxes && object.boxes === boxes, 'Typed source resources were mixed');
let rejected = false;
try { viewer.update({ ...pointFrame, objects: [{ ...pointFrame.objects[0], id: 7 }] }); } catch { rejected = true; }
check(rejected && boxes.geometry.instanceCount === 1, 'Grid accepted a point stream');
...
viewer.update(pointFrame); viewer.update(lineFrame);
viewer.setVisible(8, false);
check(!boxes.visible && !viewer.objects.get(8).points.visible && !viewer.objects.get(8).lines.visible, 'Source visibility was not independent');
...
check(!viewer.objects.has(7) && viewer.objects.get(8).points.geometry.drawRange.count === 1 && viewer.objects.get(8).lines.geometry.drawRange.count === 2,
...
check(viewer.picker.count === 1 && viewer.objects.get(7).boxes.geometry.instanceCount === 0 && viewer.objects.get(7).boxes.solid.visible, 'Cells reconnect reset controls or selection');
viewer.clearStream('points'); viewer.clearStream('cells');
check(viewer.objects.get(8).streams.size === 1, 'Lines-only source was removed');
...
check(viewer.objects.get(8).points.geometry.drawRange.count === 1 && !viewer.objects.get(8).visible, 'Points-only source lost its visibility on reconnect');
viewer.dispose(); container.remove();
return 'PASS: cells-only rendering/picking, independent geometry updates/removal, separate source bounds, reconnect camera/controls/selection persistence';
