...
These exercise ring-buffer frame spans and wraparound, IMU pairing, confidence expiry, real GLIM CPU estimation on stationary and moving synthetic scenes, map saving, and session restart/reset. The motion test translates 0.5 m and rotates 23 degrees before returning, with independently synthesized 200 Hz IMU measurements. Live sensor calibration and trajectory accuracy still require a hardware run.

For the web UI and complete camera example, see [the GLIM viewer](../html/viewer/_GLIM/README.md).
`jsonCfg/GLIM_orbbec.json` connects camera → input cloud → GLIM → map cloud →
`_WebGeometryBase` and routes Start/Stop/Reset/status through `_WSconsole`.
